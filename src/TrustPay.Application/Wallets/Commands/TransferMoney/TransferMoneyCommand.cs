...
public class TransferMoneyCommandHandler : IRequestHandler<TransferMoneyCommand, Result>
{
    ...
    public async Task<Result> Handle(TransferMoneyCommand command, CancellationToken cancellationToken)
    {
        ...
        if (walletSender.UserId != _currentUserService.UserId && !_currentUserService.IsAdmin)
        {
            return Result.Failure("У вас нет прав на совершение переводов с данного кошелька.");
        }
        ...
        var transactionResult = Transaction.CreateTransfer(walletSender.Id, walletReceive.Id, money);
